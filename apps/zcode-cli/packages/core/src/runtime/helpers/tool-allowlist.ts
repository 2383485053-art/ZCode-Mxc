...
function appendChildControlTool(
  ...
  if (config.taskType === "subagent_child") {
    ...
    // team_send/task_*（成员可用的读写看板）与 RespondToCoordinator 同类补回（四审②）：
    // profile 显式 allowedTools 与 explore 交集都会滤掉端口门控工具——allowlist 只表达
    // 「不挡」，授权仍在注册门（deps.teamPort 在场才注册）。非团队成员 child 多几个名字
    // 不会注册，无害。lead 独占的 task_create/team_collect 不补——成员不该见。
    const appended = new Set(allowlist);
    appended.add(RESPOND_TO_COORDINATOR_TOOL_NAME);
    appended.add(TEAM_SEND_TOOL_NAME);
    appended.add(TASK_LIST_TOOL_NAME);
    appended.add(TASK_QUERY_TOOL_NAME);
    appended.add(TASK_UPDATE_TOOL_NAME);
    return [...appended];
  }
